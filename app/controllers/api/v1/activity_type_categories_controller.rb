module Api
    module V1
        class ActivityTypeCategoriesController < ApplicationController
            before_action :set_object, only: [:show, :update, :destroy]

            def index
                render json: Paginator.paginated_response(params, ActivityTypeCategory.all.order(id: :asc), ["images"])
            end
